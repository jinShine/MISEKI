...
final class MainViewController: UIViewController {
    ...
    let sharingButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(didTapSharing(_:)), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    private func setupNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(updateRegion(notification:)), name: NSNotification.Name.init(App.Define.updateRegion.rawValue), object: nil)
    }
...
extension MainViewController {
    ...
    @objc private func updateRegion(notification: Notification?) {
        locationManager?.reverseGeocoderLocation { [weak self] placeMark in
            DispatchQueue.main.async {
                if let region = notification?.userInfo?[App.Define.updateRegion.rawValue] as? Juso {
    ...
    @objc private func didTapLocation(_ sender: UIButton) {
        present(Navigator.searchedAddress .viewController, animated: true, completion: nil)
    }
